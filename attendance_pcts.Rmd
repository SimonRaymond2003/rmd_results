...
```{r}
run_attendance_analysis_v2 <- function(file_path, year) {
    cat("\n\n========================================")
    cat(sprintf("\nAnalyzing %s vs 2020\n", year))
    cat("========================================\n\n")
    ...
    # Remove home_attendance and attendance_raw
    attendance_data$home_attendance <- NULL
    attendance_data$attendance_raw <- NULL
    
    # Create interaction term
    attendance_data$interact_HxA <- attendance_data$attendance_pct * attendance_data$posteam_type_home
    
    # Reorder columns
    other_cols <- setdiff(names(attendance_data), 
                         c("attendance_pct", "posteam_type_home", "interact_HxA"))
...
# Run analysis for each year using Version 2
for (year in years) {
    file_path <- sprintf("predict_attendance_%s_vs_2020.csv.gz", year)
    if (file.exists(file_path)) {
        results_list_v2[[as.character(year)]] <- run_attendance_analysis_v2(file_path, year)
    } else {
